...
/**
 * TaskFeedView — the middle "filter rail" + right "article feed" panels of
 * the three-column /tasks layout. Used by both:
 *
 *   - ``/tasks`` (triggerId=null)              → cross-trigger merged feed
 *     hits ``GET /api/news``.
 *   - ``/tasks/[id]`` (triggerId=<id>)         → per-trigger feed
 *     hits ``GET /api/triggers/{id}/news``.
 *
 * The left "trigger list" sidebar is owned by ``tasks/layout.tsx``.
 *
 * Per-ticker mode (``triggerId === "trg-news-002"``) is preserved: the
 * filter rail surfaces a per-ticker rail with article counts + earnings
 * countdowns, and the right column filters down to the chosen symbol.
 */
...
export interface TaskFeedViewProps {
  /** Trigger ID to scope the feed to, or null for the "全部" merged view. */
  triggerId: string | null;
}

export function TaskFeedView({ triggerId }: TaskFeedViewProps) {
  ...
  // Per-ticker mode only — see the comment in the "All" section below for
  // why this is keyed on the literal trigger ID rather than a richer
  // capability flag on the Trigger record.
  ...
  // Reset tag filter + selected symbol when the trigger changes so the new
  // view starts clean. The taxonomy itself doesn't need to refetch (tag
  // groups are global, not per-trigger).
  useEffect(() => {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    setError(null);
    try {
      // Per-ticker mode pulls a wider window so we can group + filter
      // client-side. Otherwise 100 is plenty for the feed.
      const limit = isPerTicker ? "1000" : "100";
  ...
  }, [isPerTicker, selectedTagIds, triggerId]);
        ...
        <div className="mb-3 flex items-center justify-between font-mono text-[10px] tracking-[0.08em] text-[var(--ink-faint)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="重新加载"
            >
              <RefreshCw size={11} className={loadingArticles ? "animate-spin" : ""} />
              刷新
            </button>
          </div>
        </div>
