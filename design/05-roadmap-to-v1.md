...
| ✅ 1.9 | Run.status 重塑 | T19 4/4 pass；Run / RunRow / SqliteRunStore 加 3 字段；`derive_overall_assessment(harness_status, evaluator_decision)` 纯函数映射成 5 个状态（running/passed/below_quality_bar/failed/ok_no_judge）；harness 在 finish 时读 eval-report.json 自动填 evaluator_decision；legacy `status` 字段保留作为 harness_status 的别名（已有 68 个 e2e 都不动）；SQLite ADD COLUMN 自动迁移 + status→harness_status backfill |
