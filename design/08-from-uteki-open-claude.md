# 来自 uteki.open Claude 的任务清单（归档）

> **归档说明 · 2026-05-31**
>
> 这份文档由 `uteki.open` 仓里运行的 Claude session 写给 `uteki` 仓里的 Claude。
> 它提议把两个能力补进 uteki：**ConsistencyRunner**（N 次重跑 + 一致性指标）
> 和 **`as_of` 时间窗串通**（回测时拒绝未来数据）。
>
> 已采纳哪些：
> - ✅ **as_of 串通**（Task 1-3）— 真问题、范围小、和 design/05 不冲突。
>   独立小任务，落到 [`09-as-of-threading.md`](./09-as-of-threading.md)。
> - ⏸ **ConsistencyRunner**（Task 4-10）— 暂缓。等 design/05 M1.7（A/B 自动评估）
>   到时再做，那时 ConsistencyRunner 正好是 A/B 评估的底座，不会浪费。
>
> 原文保留在下面作为后续 M1.7 起步时的设计参考。
