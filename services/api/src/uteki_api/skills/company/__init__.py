...
- Gate conclusion: 80-120 字，必含一句话定性 + 关键依据 [src:N]
...
- 每个 Q 的"分析"段 **80-150 字** —— "缺数据" 三个字不算回答
- 如果该 Q 缺数据(评 0 分)，分析段必须说明：
  - 缺哪类具体数据(例:DEF 14A 高管薪酬段 / 财报电话会议 Q&A)
  - 我尝试了哪些证据来源(例:已查 sec_fundamentals filings 索引、insider_trading)
  - 为什么从已有证据无法间接推断
- ❌ 禁止: "缺数据 [src:none]。" 这种 5 字答案
- ✓ 正确: "证据中无电话会议 transcript [src:none]，无法判断管理层在 AI 业务承压时的沟通透明度。但 insider_trading 中 CFO 在 H1 2025 净卖出 $9.8M [src:11] 显示对 Q3 后展望的保留态度，间接信号管理层并非'坏消息隐瞒'，给 4 分而非 0 分。" (~110 字)
...
- 总 Analysis 段 1000-1800 字
- Gate conclusion 80-120 字，必须给出"宽度+趋势"二维定性 + 关键依据
...
- Gate conclusion 80-120 字 + 综合评分 0-10
- ❌ 不允许"数据缺失"作为 1 句话答案 —— 必须说明:已查了哪些来源(insider_trading / sec_fundamentals filings / news_search)、缺什么类型证据(DEF 14A proxy / earnings call transcript / 离职新闻)
...
- 韧性评分必须给出 0-10 数字 + 至少 3 个支持理由
- 认知偏差至少列 3 种（行业 anchoring / recency bias / narrative fallacy 等),每种 50-100 字
- 最悲观情景叙述 200-400 字 — 一段连续叙述,不是要点列表
- 总 Analysis 段 1200-2000 字
- Gate conclusion 80-120 字,给出"主要威胁是 X" + "若 X 发生则 Y" 的因果链
...
- 定量锚点部分:PE / PB / PS / FCF Yield / 同行对比 这 5 项必须全部尝试给出 — 缺数据要说明缺什么 + 已用了哪些工具
- 同行对比必须列出 **至少 2 家** 可比公司 + 各自具体估值倍数
- 总 Analysis 段 800-1500 字
- Gate conclusion 80-120 字,给出 cheap/fair/expensive/bubble 四级判定之一 + 主要依据
